...
def cat_eda_plot(df, feature, output_path, style='bmh'):
	...
	plt.savefig('{}/{}_barplot.png'.format(output_path, feature), bbox_inches='tight')
	plt.close()
...
def num_eda_plot(df, feature, output_path, style='bmh'):
	...
	plt.savefig('{}/{}_hist.png'.format(output_path, feature), bbox_inches='tight')
	plt.close()


def multivariate_eda_plot(df, output_path, feature1=None, feature2=None, style='bmh'):
	'''
	...
	if (feature1==None) and (feature2==None):
		...
		plt.savefig('{}/df_heatmap.png'.format(output_path), bbox_inches='tight')
		plt.close()
		...
		plt.savefig('{}/{}_vs_{}.png'.format(output_path, feature1, feature2), bbox_inches='tight')
		plt.close()
	else:
...
def encoder_helper(df, category_lst, response):
	...
	for feat in category_lst:
		...
		df['{}_{}'.format(feat, response)] = df[feat].apply(lambda x: groups.loc[x])
...
def perform_feature_engineering(df, numeric_lst, category_lst, response):
	'''
	Perform encoding of categorical features using encoder_helper() function and split data into test and training
...
def save_model(model, model_name, output_path):
	...
	:return: None
	'''
	joblib.dump(model, '{}/{}.pkl'.format(output_path, model_name.lower().replace(' ', '_')))
...
def classification_report_image(y_train, y_test,
	...
	plt.axis('off')
	plt.savefig('{}/{}_classification_report.png'.format(
		output_path, model_name.lower().replace(' ', '_')), bbox_inches='tight')
	plt.close()


def roc_curve_image(rfc, lrc, X_test, y_test, output_path):
	'''
	Produce ROC curve plot for Logistic Regression and Random Forest Classifier models and store plot as image
...
def feature_importance_plot(model, X_data, output_path):
	...
	plt.savefig('{}/feature_importances.png'.format(output_path), bbox_inches='tight')
	plt.close()
...
def main():
	...
	multivariate_eda_plot(df, './images/eda/multivariate', feature1='Customer_Age', feature2='Total_Trans_Amt')
	multivariate_eda_plot(df, './images/eda/multivariate', feature1='Credit_Limit', feature2='Months_on_book')
	multivariate_eda_plot(df, './images/eda/multivariate')

	# Feature Engineering
	X_train, X_test, y_train, y_test = perform_feature_engineering(df, quant_cols, cat_cols, 'Churn')

	# Train Models
	lrc, y_train_preds_lr, y_test_preds_lr = train_logistic_reg(X_train, X_test, y_train)
	cv_rfc, y_train_preds_rf, y_test_preds_rf = train_random_forest(X_train, X_test, y_train)
	# Save Models
	save_model(lrc, 'Logistic Regression', './models')
	save_model(cv_rfc, 'Random Forest', './models')

	# Classification Report
	classification_report_image(y_train, y_test, y_train_preds_lr, y_test_preds_lr,
								'Logistic Regression', './images/results')
	classification_report_image(y_train, y_test, y_train_preds_rf, y_test_preds_rf,
								'Random Forest', './images/results')

	# ROC Curve
	roc_curve_image(cv_rfc, lrc, X_test, y_test, './images/results')

	# Feature Importance
	feature_importance_plot(cv_rfc.best_estimator_, pd.concat((X_train, X_test), axis=0),
